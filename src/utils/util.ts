...
const HOME_ORIENTATION: THREE.Quaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(15 * Math.PI / 180, -20 * Math.PI / 180, 0));
const cubeQuaternion: THREE.Quaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(30 * Math.PI / 180, -30 * Math.PI / 180, 0));
...
function faceletsToPattern(facelets: string): KPattern {
    ...
    return new KPattern(kPuzzle333, patternData);

}

const prepareMoves = (moves: any[]) => {
    const lastMoveString = moves.join(" ")
